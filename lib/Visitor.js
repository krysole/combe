...
Visitor.prototype.visitChildren = function (ast) {
  ...
  for (var i = 0; i < ast.principleAttributeNames.length; i++) {
    result[ast.principleAttributeNames[i]] = this.visit.apply(this, [ast.principleAttributeNames[i]].concat(rest));
  }
...
Visitor.prototype.handle = function (ast) {
  ...
  if (handler != null) {
    ...
    return this._defaultHandleUnspecified.apply(this, arguments);
  }
  else if (this.unspecified === "error") {
    return new Error("Unspecified handler for ast " + Show.synopsis(ast));
  }
...
Visitor.prototype._defaultHandleUnspecified = function (ast) {
