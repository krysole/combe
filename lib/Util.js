...
Util.equal = function (left, right) {
  if (left === null) {
    return right === null;
  }
  else if (left === undefined) {
    return right === undefiend;
  }
    ...
    for (var name in left) {
      if (!Util.equal(left[name], right[name])) return false;
    }
    return true;
  }
  else if (left.__combe_equal != null && left.__combe_equal === right.__combe_equal) {
    return left.__combe_equal(right);
  }
  else {
    return false;
  }
