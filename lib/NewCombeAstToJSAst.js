...
var NewCombeAst_SimplifyPatterns = require('./NewCombeAst_SimplifyPattern');
...
var NewCombeAstToJSAst = module.exports = Class.new(Object, {
  ...
  emitFunctionFromExpression: function (ast, args) {
    return JSAst.Function(null, [], [
      JSAst.Return(expression)
    ]);
  },
  
  emitReturnInvokeRule: function (expression) {
    if (args == null) args = [];
  ...
  visitRule: function (ast) { // [ name, parameters, pattern ]
    ...
    statements.push(this.emitReturnInvokeRule(ast.pattern.visit(this)));
