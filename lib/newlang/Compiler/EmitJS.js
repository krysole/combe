...
var EmitJS = module.exports = Class.new(Object, {
  ...
  visitPropertyAssignment: function (ast) {
    // Todo
  },
  ...
  visitPropertyLookup: function (ast) {
    // Todo
  },
  ...
  visitFunction: function (ast) {
    ...
    if (ast.parameters.some(function (paramNode) {
      ...
      var params = ast.parameters.map(function (paramNode) {
        return (paramNode.name != null ? paramNode.name : '__combe__unnamedParameter');
      }).interpolate(', ');
      ast.code = [
        '(function (', params, ') {\n',
          'return (', ast.body.code, ');\n',
  ...
  visitDefineMethod: function (ast) {
    // Todo
  },
