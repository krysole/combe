...
var EmitJS = module.exports = Class.new(Object, {
  ...
  visitSelf: function (ast) {
    // Todo: Make sure that self is set in all methods correctly. Note that
    // self is not automatically provided for arbitary functions, only functions
    // defined as a method have a synthesised self binding (i.e., methods properties
  ...
  visitFunction: function (ast) {
    // Todo: This is probably a good place to start next time...
    
    // These need to support the exception based return mechanism (non-local, 
    // since Newlangs control structures are all implemented using JS functions
    // and therefore JS's return operation won't work).
    
    // The splat mechanism suggests a simple form of pattern matching over
    // parameters. It would probably be best if I can get this working without
    // pattern objects to start with. If it can be made to work by using an 
    // index and some conditional logic that should be enough.
