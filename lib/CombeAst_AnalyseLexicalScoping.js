...
var CombeAst_AnalyseLexicalScoping = module.exports = Class.new(Object, {
  ...
  visitFunctionDeclaration: function (ast) {
    if (ast.name) this.scope.variables.pushIfAbsent(ast.name);
    
    
    ast.lexicalScope = this.pushScope(ast);
    
    this.scope.variables.pushAll(ast.parameters);
  ...
  visitSetPropertyDeclaration: function (ast) {
    ...
    this.scope.variables.push(ast.parameter);
  ...
  visitVariableDeclaration: function (ast) {
    this.scope.variables.pushIfAbsent(ast.name);
  ...
  visitVariablePattern: function (ast) {
    ast.containingScope = this.scope;
  },
