...
Translator.prototype.handleDotMethodCallPattern = function (ast) {
  var pattern = this.visit(ast.pattern);
  var arguments = this.visitArray(ast.arguments).join(",\n");
  
  return (
    "function () {\n" +
    "  return " + pattern + ".call(this)." + ast.name + "(" + arguments + ");\n" +
    "}"
  );
};

Translator.prototype.handleDotPattern = function (ast) {
  var pattern = this.visit(ast.pattern);
  
  return (
    "function () {\n" +
    "  return " + pattern + ".call(this)." + ast.name + ";\n" +
    "}"
  );
};

Translator.prototype.handleSubscriptMethodCallPattern = function (ast) {
  var pattern = this.visit(ast.pattern);
  var name = this.visit(ast.nameFragment);
  var arguments = this.visitArray(ast.arguments).join(",\n");
  
  return (
    "function () {\n" +
    "  return " + pattern + ".call(this)[" + name + "](" + arguments + ");\n" +
    "}"
  );
};

Translator.prototype.handleSubscriptPattern = function (ast) {
  var pattern = this.visit(ast.pattern);
  var name = this.visit(ast.nameFragment);
  
  return (
    "function () {\n" +
    "  return " + pattern + ".call(this)[" + name + "];\n" +
    "}"
  );
};

Translator.prototype.handleCallPattern = function (ast) {
  var pattern = this.visit(ast.pattern);
  var arguments = this.visitArray(ast.arguments).join(",\n");
  
  return (
    "function () {\n" +
    "  return " + pattern + ".call(this)(" + arguments + ");\n" +
    "}"
  );
};
...
Translator.prototype.handleImmediatePattern = function (ast) {
  var body = this.visit(ast.semanticBody);
  
  return (body + ".call(this)");
};

Translator.prototype.handlePropertyPattern = function (ast) {
