...
// Depends on NodeJS APIs (including the console global)

define(['fs', 
        './CombeJSParser', 
        './CombeJSAstRemoveUnnecessaryNodes',
        './CombeJSAstAnalyseDeclare', 
        './CombeJSAstToJSAst', 
        './JSParser', 
        'util']
       funciton (fs, CombeJSParser, CombeJSAstRemoveUnnecessaryNodes,
                 CombeJSAstAnalyseDeclare, CombeJSAstToJSAst, 
                 JSParser, util) {
  'use strict';
  
  var inspect = util.inspect;
  var JSAstToJSSource = JSParser.JSAstToJSSource;
  
  return {
    compile: function (sourceFile, destFile) {
      if (!destFile) {
      ...
      var jsOutput = Array.deepJoin(iolist);
