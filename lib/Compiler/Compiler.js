...
Compiler.parse = function (source, sourcename) {
  return new CombeParser().match(source, sourcename);
};

Compiler.analyse = function (ast0) {
  var ast1 = new AnalyseBindings().visit(ast0);
  return ast1;
};

Compiler.translate = function (ast) {
  return new TranslateAstToJS().visit(ast);
};

Compiler.compile = function (source, sourcename) {
  var hashbang = this.hashbang(source);
  var ast = this.parse(source.slice(hashbang.length), sourcename);
...
Compiler.astToSingleline = function (object, column) {
  var result;
  if (e === null) result = " ".repeat(column) + "null";
  else if (e === undefined) result = " ".repeat(column) + "undefined";
  else if (e instanceof Array) result = Compiler.astArrayToSingleline(e, 0);
  else if (e instanceof String) result = " ".repeat(column) + e.quote();
  else if (e instanceof Number) result = " ".repeat(column) + e;
  else if (e instanceof RegExp) result = " ".repeat(column) + e;
  else if (e.constructor != null && e.constructor.name != null &&
           e.toString() === "[object Object]") {
    result = " ".repeat(column) + "[object " + e.constructor.name + "]";
  }
    ...
    result = " ".repeat(column) + e.toString();
  }
...
Compiler.astToMultiline = function (ast, column) {
  if (e === null) return " ".repeat(column) + "null";
  else if (e === undefined) return " ".repeat(column) + "undefined";
  else if (e instanceof Array) return Compiler.astArrayToMultiline(e, 0);
  else if (e instanceof String) return " ".repeat(column) + e.quote();
  else if (e instanceof Number) return " ".repeat(column) + e;
  else if (e instanceof RegExp) return " ".repeat(column) + e;
  else if (e.constructor != null && e.constructor.name != null &&
           e.toString() === "[object Object]") {
    return " ".repeat(column) + "[object " + e.constructor.name + "]";
  }
  else if (Object.getPrototypeOf(e) === Object.prototype &&
           Object.getPrototypeOf(e) === null) {
    ...
    return " ".repeat(column) + e.toString();
  }
};
