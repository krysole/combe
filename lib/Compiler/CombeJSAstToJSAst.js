...
var JSAstToJSSource = require('../JSParser/JSAstToJSSource');
...
var CombeJSAstToJSAst = module.exports = Class.new(Object, {}, {
  ...
  processRule: function (ast, inherited, declaration) {
    ...
    if (ast.argumentNames == null || ast.argumentNames.isEmpty()) {
      newAst = this.funcWithExpression(
        this.callThisProperty('memoize', [
          Ast('ValueLiteral', {
            value: ast.name ? ast.name : this.gensymWithHash('unnamed', newAst)
          }),
          newAst
        ])
      );
  ...
  visitAnythingPattern: function (ast, inherited) {
    return {replace:
      Ast('DotPropertyLookup', {name: 'anything'}, Ast('This'))};
  },
  ...
  gensym: function (string) {
    return string + '_' + Math.generateSalt(8);
  },
  
  gensymWithHash: function (string, ast) {
    return string + '_' + this.hashCode(ast);
  },
  
  hashCode: function (ast) {
    var codeString = JSAstToJSSource.new().translate(ast).deepJoin();
    var hash = crypto.createHash('md5');
    hash.update(codeString);
    var hashString = hash.digest('base64');
    return hashString.slice(0, -2);
  },
