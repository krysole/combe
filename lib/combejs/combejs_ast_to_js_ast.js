...
util.extend(CombeJSAstToJSAst.prototype, {
  ...
  visitInclusiveRange: function (ast, inherited) {
    ...
    return {replace:
      Ast('CallProperty', {name: 'inclusive'},
  ...
  visitExclusiveRange: function (ast, inherited) {
    var children = this.visitAllReplacing(ast.children, {});
    return {replace:
      Ast('CallProperty', {name: 'exclusive'},
        Ast('VariableLookup', {name: '__combejs__Range'})).concat(children)};
  },
  ...
  visitRuleExpression: function (ast, inherited) {
    ...
    var replace = Ast('FunctionExpression', 
      {name: ast.name, argumentNames: ast.argumentNames});
    if (!vardecls.isEmpty()) {
      replace.push(Ast('VariableDeclarationList').concat(vardecls));
    }
    replace.push(
      Ast('Return', {}, 
  ...
  visitRuleDeclaration: function (ast, inherited) {
    ...
    var replace = Ast('FunctionDeclaration', 
      {name: ast.name, argumentNames: ast.argumentNames});
    if (!vardecls.isEmpty()) {
      replace.push(Ast('VariableDeclarationList').concat(vardecls));
    }
