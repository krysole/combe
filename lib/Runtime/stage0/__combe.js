...
global.__combe_defineSetProperty = function (subject, name, value) {
  var descriptor = { get: null, set: value, enumerable: true, configurable: true };
  if (Object.hasOwnProperty(subject, name)) {
    var existingDescriptor = Object.getOwnPropertyDescriptor(subject, name);
...
Object.prototype.__combe_initializeMemoTableArray = function (size) {
  assert(this.__combe_memoTableArray_storage == null);
  this.__combe_memoTableArray_storage = Array.new(size);
};
...
Object.prototype.__combe_memoize = function (name, parser) {
  ...
  if (entry != null && entry.initialState === this.state) {
    if (entry.result === Backtrack) {
      throw Backtrack;
    }
    else {
      this.state = entry.finalState;
      this.position = entry.finalPosition;
      return entry.result;
    }
  }
  else {
    ...
    try {
      ...
      memoTable[name] = { result: result, finalPosition: this.position, initialState: initialState, finalState: this.state };
      return result;
    }
    catch (e) {
      if (e === Backtrack) {
        memoTable = { result: Backtrack, finalPosition: -1, initialState: initialState, finalState: initialState };
      }
      throw e;
    }
  }
