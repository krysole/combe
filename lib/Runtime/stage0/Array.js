...
Array.extend({
  ...
  remove: function (what) {
    for (var i = 0; i < this.length; i++) {
      if (__combe_infixOperators['=='](this[i], what)) {
  ...
  toString: function () {
    return [
      '[',
        this.map(String.stringFrom).separatedBy(', '),
      ']'
    ].deepJoin();
  },
  ...
  toSourceString: function () {
    return [
      '[',
        this.map(String.sourceStringFrom).separatedBy(', '),
      ']'
    ].deepJoin();
  },
  ...
  toCombeSourceString: function () {
    return [
      '[',
        this.map(String.combeSourceStringFrom).separatedBy(', '),
      ']'
    ].deepJoin();
  },
