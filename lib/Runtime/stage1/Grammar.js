...
global.Grammar = Object.subclass('Grammar', {
  ...
  pushState: function (name, value) {
    ...
    this.state[name] = (this.state[name] != null) ? this.state[name].copy() : [];
  ...
  popState: function (name) {
    this.state = Object.clone(this.state);
    this.state[name] = this.state[name].copy();
  ...
  setState: function (name, positionFromTop, value) {
    this.state = Object.clone(this.state);
    this.state[name] = this.state[name].copy();
    
    if (value == null) { value = positionFromTop; positionFromTop = 0; }
    return this.state[name][this.state[name].length - positionFromTop - 1] = value;
  },
