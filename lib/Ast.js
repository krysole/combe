...
var Ast = module.exports = Class.new(Object, {
  ...
  addTypeConstructors: function (constructor, types) {
    var self = this;
    Object.getOwnPropertyNames(types).each(function (name) {
      self[name] = function () {
        var args = Array.slice(arguments);
        return self.new(name, types[name], args);
      };
    });
  },
  ...
  initialize: function (type, childrenNames, children) {
    this.type = type;
    this.childrenNames = childrenNames;
    for (var i = 0; i < childrenNames.length; i++) {
      this[childrenNames[i]] = children[i];
    }
  },
    ...
    this.childrenNames.each(function (name) {
      if (Array.isArray(self[name])) {
        values.pushAll(self[name].filter(function (elem) {
          return elem instanceof MetaphorAst;
        }));
      }
      else if (self[name] instanceof MetaphorAst) {
  ...
  visit: function (visitor) {
    ...
    if (visitorMethod) {
      ...
      throw new Error('No visitor method specified for MetaphorAst type: ' + this.type);
    }
  },
  
  visitChildren: function (visitor) {
    return this.allChildren().visitAll(visitor);
  },
