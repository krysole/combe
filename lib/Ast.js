...
function Ast(properties, principleAttributeNames) {
  ...
  this.principleAttributeNames = principleAttributeNames || [];
...
Ast.define = function (name, table) {
  ...
  var Constructor = new Function(
    ...
    "    var attributeNames = " + name + ".type[type];\n" +
    "    if (rest.length > attributes.length) {\n" +
    ...
    "    }\n" +
    "    \n" +
    "    Ast.call(this, attributes, attributeNames);\n" +
    "  }\n" +
    "  else {\n" +
...
Ast.prototype.__combe_showSingleline = function (column) {
  var result = " ".repeat(column) + "Ast " + ast.type + " {";
  for (var i = 0; i < this.principleAttributeNames.length; i++) {
    if (i > 0) result += ", ";
...
Ast.prototype.__combe_showMultiline = function (column) {
  var head = " ".repeat(column) + "Ast " + ast.type + " {...}";
  var lines = [];
  for (var i = 0; i < this.principleAttributeNames.length; i++) {
    ...
    if (!Show.shouldBreak(singleline)) {
      ...
      lines.push(Show.show(this[this.principleAttributeNames[i]], column + 2));
    }
  }
  for (var name in this) {
    if (this.hasOwnProperty(name) && !this.principleAttributeNames.includes(name)) {
      ...
      if (!Show.shouldBreak(singleline)) {
        ...
        lines.push(Show.show(this[name], column + 2));
      }
    }
  }
  return head + "\n" + lines.join("\n");
