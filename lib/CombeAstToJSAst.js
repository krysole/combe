...
var CombeAstToJSAst = module.exports = Class.new(Object, {
  ...
  visitSetProperty: function (ast) {
    return JSAst.SetProperty(ast.name, ast.argumentName, this.visitAll(ast.statements));
  },
  ...
  visitRule: function (ast) {
    ...
    if (declareLocals.length !== 0) {
      stmts.push(JSAst.Var(declareLocals.map(function (elem) {
        return JSAst.VariableDeclaration(elem, null);
      })));
    }
    
    stmts.push(JSAst.Return(this.generateCallRule(this.visit(ast.pattern))));
    
    var ruleFunction = JSAst.Function(null, ast.argumentNames, stmts);
    
    if (ast.argumentNames.length === 0) {
      ruleFunction = this.generateRuleFromExpression(
        this.generateCallRuleNamed('memoize', [
  ...
  visitConcatPattern: function (ast) {
    ...
    var stmts = patterns.slice(0, patterns.length - 1).map(function (elem) {
      return JSAst.ExpressionStatement(self.generateCallRule(elem));
    });
    stmts.push(JSAst.Return(self.generateCallRule(patterns.last)));
  ...
  visitPredicatePattern: function (ast) {
    return this.generateRuleFromExpression(
      this.generateCallRuleNamed('_predicate', [this.visit(ast.body)])
    );
  },
  
  visitImmediatePattern: function (ast) {
    return this.generateCallRule(this.visit(ast.body));
  },
  ...
  hashCode: function (ruleFunctionAst) {
    ...
    return hashString.slice(0, -2); // Remove base64 suffix
  },
