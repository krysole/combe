...
Object.prototype.predicate = function (predicate) {
  if (predicate.call(this)) return null;
  else throw Backtrack;
};

Object.prototype.action = function (action) {
  return action.call(this);
};

Object.prototype.nested = function (pattern) {
  var input = this.next();
...
Grammar.prototype.peek = function () {
  var position = this.getPosition();
  var input = this.getInput();
  if (position < input.count()) return input.at(position);
  else                          throw Backtrack;
};
...
Grammar.prototype.last = function () {
  var position = this.getPosition();
  var input = this.getInput();
  if (position > 0) return input.at(position - 1);
  else              throw Backtrack;
};
...
Grammar.prototype.rest = function () {
  var position = this.getPosition();
  var input = this.getInput();
  this.setPosition(input.count());
  return input.slice(position, input.count());
};

Grammar.prototype.end = function () {
  var position = this.getPosition();
  var input = this.getInput();
  if (position === input.count()) return null;
  else                            throw Backtrack;
};
...
Grammar.prototype.error = function (message) {
  throw new Error("[" + this.getName() + " at " + this.positionString() + "] " + message);
};



Grammar.prototype.getMemoTableForPosition = function (position) {
  return null; // Grammar does not support memoization by default.
};
...
Grammar.prototype.memoize = function (name, pattern) {
  ...
  if (memoTable != null) {
    ...
    if (memo != null) {
      ...
      var dynamicVariables = this._dynamicVariables;
      try {
        var result = pattern.call(this);
        if (this._dynamicVariables === dynamicVariables) { // Only memoize if there are no outgoing dynamic variable side effects.
          memoTable[name] = { result: result, position: this.getPosition() };
        }
        return result;
      }
      catch (e) {
        if (e === Backtrack && this._dynamicVariables === dynamicVariables) { // Only memoize if there are no outgoing dynamic variable side effects.
          memoTable[name] = Backtrack
        }
        throw e;
      }
    }
...
Grammar.prototype.log = function (message) {
  console.log('%s log() at %s; %s', this.getName(), this.positionString(), message);
};
