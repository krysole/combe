...
Grammar.prototype.log = function (message) {
  console.log('%s log() at %s; %s', this._name(), this._positionString(), message);
};
...
Grammar.prototype.emit = function (pattern) {
...
Grammar.prototype.end = function () {
  if (this.getPosition() === this.getInput().length) return null;
  else                                               throw Backtrack;
};
...
Grammar.prototype._memoize = function (name, pattern) {
  ...
  //       memosForPosition[name] = { result: result, position: this.getPosition() };
  //     }
  //     return result;
  //   }
  //   catch (e) {
...
Grammar.prototype.set = function (name, object) {
  ...
  this._state[name] = new List(object, this._state[name].tail);
  return object;
};



Grammar.prototype._name = function () {
...
Grammar.prototype.toString = function () {
  if (this._name() != null) {
    return "[object Combe.Grammar " + this._name + "]";
  }
  else {
